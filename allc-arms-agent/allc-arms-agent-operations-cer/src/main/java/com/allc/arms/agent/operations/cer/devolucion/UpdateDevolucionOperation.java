...
public class UpdateDevolucionOperation extends AbstractOperation {
	private static Logger logger = Logger.getLogger(UpdateDevolucionOperation.class);
	...
	public boolean process(ConnSocketServer socket, Frame frame, PropFile properties) {
		logger.info("Update Devolucion Operation iniciada...");

		try {
			Files.creaEscribeDataArchivo4690(getEyesFileName(properties), "UPD_DEV_O|"+properties.getHostName()+"|1|"+properties.getHostAddress()+"|"+frame.getHeader().get(3)+"|STR|"+ArmsAgentConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|Actualizando devolución: "+frame.getBody().get(0)+".\n", true);
			...
			if (socketClient.connectSocketUsingRetries() && socketClient.writeDataSocket(trama)) {
				...
				if(devEncontrada)
					Files.creaEscribeDataArchivo4690(getEyesFileName(properties), "UPD_DEV_O|"+properties.getHostName()+"|1|"+properties.getHostAddress()+"|"+frame.getHeader().get(3)+"|END|"+ArmsAgentConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|Devolución: "+frame.getBody().get(0)+" actualizada.\n", true);
				else
					Files.creaEscribeDataArchivo4690(getEyesFileName(properties), "UPD_DEV_O|"+properties.getHostName()+"|1|"+properties.getHostAddress()+"|"+frame.getHeader().get(3)+"|END|"+ArmsAgentConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|Devolución: "+frame.getBody().get(0)+" no actualizada.\n", true);
			} else
				Files.creaEscribeDataArchivo4690(getEyesFileName(properties), "UPD_DEV_O|"+properties.getHostName()+"|1|"+properties.getHostAddress()+"|"+frame.getHeader().get(3)+"|WAR|"+ArmsAgentConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|No hay conexión con el server para actualizar la Devolución: "+frame.getBody().get(0)+".\n", true);
		} catch (Exception e) {
			try {
				Files.creaEscribeDataArchivo4690(getEyesFileName(properties), "UPD_DEV_O|"+properties.getHostName()+"|1|"+properties.getHostAddress()+"|"+frame.getHeader().get(3)+"|ERR|"+ArmsAgentConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|Error al actualizar la Devolución: "+frame.getBody().get(0)+".\n", true);
			} catch (Exception e1) {
