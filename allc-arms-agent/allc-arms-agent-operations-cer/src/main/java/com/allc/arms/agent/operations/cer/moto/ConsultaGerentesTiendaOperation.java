...
public class ConsultaGerentesTiendaOperation  extends AbstractOperation{
	...
	private static Logger logger = Logger.getLogger(ConsultaGerentesTiendaOperation.class);
	private Pattern p = Pattern.compile(ArmsAgentConstants.Communication.REGEX);

	public boolean process(ConnSocketServer socket, Frame frame, PropFile properties) {
		
		logger.info("Consulta Gerentes de Tienda Operation iniciada...");

		try {
			Files.creaEscribeDataArchivo4690(getEyesFileName(properties), "CONS_GERENTE_O|"+properties.getHostName()+"|1|"+properties.getHostAddress()+"|"+frame.getHeader().get(3)+"|STR|"+ArmsAgentConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|Consultando Gerentes: "+frame.getBody().get(0)+".\n", true);
			...
			if (socketClient.connectSocketUsingRetries() && socketClient.writeDataSocket(trama)) {
				...
				if(gerenteEncontrado)
					Files.creaEscribeDataArchivo4690(getEyesFileName(properties), "CONS_GERENTE_O|"+properties.getHostName()+"|1|"+properties.getHostAddress()+"|"+frame.getHeader().get(3)+"|END|"+ArmsAgentConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|Gerentes para la tienda: "+frame.getBody().get(0)+" encontrados.\n", true);
				else
					Files.creaEscribeDataArchivo4690(getEyesFileName(properties), "CONS_GERENTE_O|"+properties.getHostName()+"|1|"+properties.getHostAddress()+"|"+frame.getHeader().get(3)+"|END|"+ArmsAgentConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|Gerentes para la tienda: "+frame.getBody().get(0)+" no encontrados.\n", true);
			} else
				Files.creaEscribeDataArchivo4690(getEyesFileName(properties), "CONS_GERENTE_O|"+properties.getHostName()+"|1|"+properties.getHostAddress()+"|"+frame.getHeader().get(3)+"|WAR|"+ArmsAgentConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|No hay conexión con el server para consultar de Gerentes para la tienda: "+frame.getBody().get(0)+".\n", true);
		} catch (Exception e) {
			try {
				Files.creaEscribeDataArchivo4690(getEyesFileName(properties), "CONS_GERENTE_O|"+properties.getHostName()+"|1|"+properties.getHostAddress()+"|"+frame.getHeader().get(3)+"|ERR|"+ArmsAgentConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|Error al buscar los Gerentes: "+frame.getBody().get(0)+".\n", true);
			} catch (Exception e1) {
