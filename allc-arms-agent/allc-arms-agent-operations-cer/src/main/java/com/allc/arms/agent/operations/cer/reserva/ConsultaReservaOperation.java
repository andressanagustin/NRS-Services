package com.allc.arms.agent.operations.cer.reserva;
...
public class ConsultaReservaOperation extends AbstractOperation{
	...
	private static Logger logger = Logger.getLogger(ConsultaReservaOperation.class);
	private Pattern p = Pattern.compile(ArmsAgentConstants.Communication.REGEX);

	public boolean process(ConnSocketServer socket, Frame frame, PropFile properties) {
		logger.info("Consulta Rerserva Operation iniciada...");

		try {
			Files.creaEscribeDataArchivo4690(getEyesFileName(properties), "CONS_RSV_O|"+properties.getHostName()+"|1|"+properties.getHostAddress()+"|"+frame.getHeader().get(3)+"|STR|"+ArmsAgentConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|Consultando Reserva para la tienda: "+frame.getBody().get(0)+".\n", true);
			...
			if (socketClient.connectSocketUsingRetries() && socketClient.writeDataSocket(trama)) {
				...
				if(tiendaEncontrada)
					Files.creaEscribeDataArchivo4690(getEyesFileName(properties), "CONS_RSV_O|"+properties.getHostName()+"|1|"+properties.getHostAddress()+"|"+frame.getHeader().get(3)+"|END|"+ArmsAgentConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|Reserva para la tienda: "+frame.getBody().get(0)+" encontrada.\n", true);
				else
					Files.creaEscribeDataArchivo4690(getEyesFileName(properties), "CONS_RSV_O|"+properties.getHostName()+"|1|"+properties.getHostAddress()+"|"+frame.getHeader().get(3)+"|END|"+ArmsAgentConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|Reserva para la tienda: "+frame.getBody().get(0)+" no encontrada.\n", true);
			} else
				Files.creaEscribeDataArchivo4690(getEyesFileName(properties), "CONS_RSV_O|"+properties.getHostName()+"|1|"+properties.getHostAddress()+"|"+frame.getHeader().get(3)+"|WAR|"+ArmsAgentConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|No hay conexión con el server para consultar reservas para la tienda: "+frame.getBody().get(0)+".\n", true);
		} catch (Exception e) {
			try {
				Files.creaEscribeDataArchivo4690(getEyesFileName(properties), "CONS_RSV_O|"+properties.getHostName()+"|1|"+properties.getHostAddress()+"|"+frame.getHeader().get(3)+"|ERR|"+ArmsAgentConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|Error al buscar reservas para la tienda: "+frame.getBody().get(0)+".\n", true);
			} catch (Exception e1) {
