package com.allc.arms.agent.processes.fav.el;
		...
		try {
			...
			if (filePointer >= 0) {
				...
				while (!endTSLProcess && !errorReadEL && !storeClosePassed) {
					...
					if ((businessDateDay!= null && !businessDateDay.equals("00000000")) && null != line && !line.trim().equals("")) {
						line = line.substring(1, line.length() - 1);
						/** Obtain the data to manipulate **/
						data = ident(line);
						if (data != null) {
							String[] decodArray = data.split("\\|");
							String type = decodArray.length > 3 ? decodArray[3].toString() : "";
							log.info("TYPE: " + type);
							if(type.equals("30") || type.equals("31") || type.equals("32")){
								String typeDesc = type.equals("30") ? "Cambio Opera Auth Record " : (type.equals("31") ? "Cambio Item Record " : "Cambio Tender Verif Record ");
								Files.creaEscribeDataArchivo4690(getEyesFileName(), "EL_READ_P|"+properties.getHostName()+"|1|"+properties.getHostAddress()+"|"+storeNumber+"|PRC|"+ArmsAgentConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|" + typeDesc + "- Type: "+decodArray[5].toString() + " - Cod: " + decodArray[6].toString() + " - Old: " + decodArray[7].toString() + " - New: " + decodArray[8].toString() +"\n", true);
								log.info(typeDesc + "- Type: "+decodArray[5].toString() + " - Cod: " + decodArray[6].toString() + " - Old: " + decodArray[7].toString() + " - New: " + decodArray[8].toString());
	...
	private String parser(List reg, int pp) {
		...
		try {
			switch (pp) {
				...
				case 30: {
					log.info("case 30");
					salida = TSLUtility.campos(reg, "011110100");
					break;
				}
				case 31: {
					log.info("case 31");
