package com.allc.arms.agent.processes.comm;
...
/**
 * Proceso encargado de enviar a la impresora configurada para cada terminal, el ticket que se encuentra en un archivo.
 * 
 * @author gustavo
 *
 */
public class SendToPrinterBySocket extends AbstractProcess {
	protected Logger log = Logger.getLogger(SendToPrinterBySocket.class);
	...
	protected boolean init() {
		...
		try {
			...
			while(store.length() < 3)
				store = "0" + store;
			inFolder = new File4690(properties.getObject("sendToPrinter.in.folder.path"));
			inFolder.mkdirs();
			timeSleep = Long.parseLong(properties.getObject("sendToPrinter.timeSleep").toString());
			result = true;
		} catch (Exception e) {
	...
	public void run() {
		log.info("Iniciando Send To Printer By Socket Process...");
		init();
		while (!isEnd) {
