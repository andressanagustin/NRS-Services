...
/**
 * Proceso encargado de actualizar solo archivos CMD que provienen del Servidor. Es decir, que actualiza desde el disco F hacia el C.
 * 
 * @author gustavo
 *
 */
public class FileCmdUpdaterDownProcess extends AbstractProcess {
	protected Logger log = Logger.getLogger(FileCmdUpdaterDownProcess.class);
	...
	protected boolean init() {
		...
		try {
			...
			inFolder = new File4690(properties.getObject("fileUpdaterDown.in.folder.path")+File4690.separatorChar+"CmdFiles");
			log.info("Nombre de directorio");
			log.info(inFolder.getPath());
			inFolder.mkdir();
	...
	public void run() {
		log.info("Iniciando File Cmd Updater Down Process...");
		init();
		while (!isEnd) {
			...
			try {
				...
				for(int i = 0; i < size; i++){
					updateFile = list[i];
					Files.creaEscribeDataArchivo4690(getEyesFileName(), "FILEC_UPD_D_P|"+properties.getHostName()+"|1|"+properties.getHostAddress()+"|"+store+"|STR|"+ArmsAgentConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|Iniciando procesamiento de: "+ updateFile.getName()+".\n", true);
					log.info("Archivo a procesar: " + updateFile.getName());
					String cmdFileName = inFolder.getAbsolutePath()+File4690.separator +updateFile.getName().split("\\.")[0]+".CMD";
					String originDir = readDirPath(cmdFileName);
					if(originDir == null){
						log.info("Archivo de comandos: " + cmdFileName + "no contiene ruta.");
						Files.creaEscribeDataArchivo4690(getEyesFileName(), "FILEC_UPD_D_P|"+properties.getHostName()+"|1|"+properties.getHostAddress()+"|"+store+"|WAR|"+ArmsAgentConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|No se pudo actualizar el archivo: "+ updateFile.getName()+".\n", true);
					} else {
						File4690 originFile = new File4690(originDir+File4690.separator+updateFile.getName());
	...
	//							log.info("Archivo: " +originDir+File.separator +originFile.getName() + " actualizado");
								...
								Files.creaEscribeDataArchivo4690(getEyesFileName(), "FILEC_UPD_D_P|"+properties.getHostName()+"|1|"+properties.getHostAddress()+"|"+store+"|END|"+ArmsAgentConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|Finalizó el procesamiento del archivo: "+ updateFile.getName()+".\n", true);
								StringBuffer data = new StringBuffer();
								data.append("S").append(ArmsAgentConstants.Communication.FRAME_SEP)
										.append(ArmsAgentConstants.Process.FILE_CMD_UPDATER_DOWN_PROCESS)
				...
				try {
					Files.creaEscribeDataArchivo4690(getEyesFileName(), "FILEC_UPD_D_P|"+properties.getHostName()+"|1|"+properties.getHostAddress()+"|"+store+"|ERR|"+ArmsAgentConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|Error al procesar el archivo: "+ updateFile.getName()+".\n", true);
				} catch (Exception e1) {
...
//		log.info("Buscando archivos para actualizar...");
		File4690[] files = inFolder.listFiles(new FileFilter4690() {
			public boolean accept(File4690 pathname) {
				return pathname.isFile() && pathname.getName().toUpperCase().endsWith(".CMD") && !pathname.getName().toUpperCase().startsWith("TMP");
			}
		});
		if (files.length != 0) {
...
//		log.info("UpdateLM: "+updateFile.lastModified());
	...
	public boolean shutdown(long timeToWait) {
		...
		log.info("Deteniendo FileCmdUpdaterDownProcess...");
		while (!finished)
			try {
