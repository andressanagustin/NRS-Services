...
import com.allc.arms.agent.processes.colas.RegistroPersonasEnColaProcessV2;
...
public class TSLReaderProcess extends AbstractProcess {
	...
	protected String tslFileName = null;
        private String tslSeekAuxFileName;
	...
	protected boolean init() {
		...
		try {
			...
			AlertListFileName = properties.getObject("alertaPersonaCola.file.listAlert");
                        tslSeekAuxFileName = properties.getObject("tslReader.file.seek.aux");
	...
	protected boolean readTSLReg() {
		...
		try {
			...
			if ((Files.fileExists4690(tslSeekFileName) && !newTslFileName.equalsIgnoreCase(obtieneNombreSeek()))
                                || storeClosedPassed) {
				changeFiles = true;
			...
			data.append(ArmsAgentConstants.Communication.SOCKET_CHANNEL)
                                .append(ArmsAgentConstants.Communication.FRAME_SEP)
                                .append(ArmsAgentConstants.Process.TSL_PROCESS)
			...
			if (punteroFile >= 0) {
				...
				while (!endTSLProcess && !isStoreClosed && !errorReadTSL) {
					...
					if (businessDateDay != null && !businessDateDay.equals("00000000") && null != linea && !linea.trim().equals("") && businessDateDay.length()==8) {
						...
						if (decod.length() > 0) {
							...
							if (frame.loadData()) {
                                                            try{
                                                                log.info("Trama a enviar: " + frame.getBody());
                                                                log.info("Intento : " + count);
                                                            ...
                                                            if (!sent) {
                                                                count ++;
                                                                    ...
                                                                    if(count == 3){
                                                                        ...
                                                                        Files.creaEscribeDataArchivo4690(tslSeekAuxFileName, trama + ArmsAgentConstants.Communication.CRLF, true);
                                                                        Files.creaEscribeDataArchivo4690(getEyesFileName(), "TSL_READ_P|"+properties.getHostName()+"|1|"+properties.getHostAddress()+"|"+storeNumber+"|ERR|"+ArmsAgentConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|"+value+"\n", true);
                                                                        sent = true;
                                                                        count = 0;
                                                                    }
                                                            }else{
                                                                count = 0;
                                                                log.info("Trama enviada " + frame.getBody());
                                                                log.info("Intento : " + count);
                                                            }
							}
							...
							if(isStoreClosed){
								try {
									...
									RegistroPersonasEnColaProcessV2.storeClosePassed = true;
                                                                        TSLReaderAuxProcess.storeClosePassed = true;
									...
									log.error(e.getMessage(), e);
                                                ...
                                                {
                                                    count ++;
                                                    log.info("Error en decod " + decod.length());
                                                    log.info("Intento : " + count);
                                                    if(count == 3){
                                                        Files.creaEscribeDataArchivo4690(getEyesFileName(), "TSL_READ_P|"+properties.getHostName()+"|1|"+properties.getHostAddress()+"|"+storeNumber+"|ERR|"+ArmsAgentConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|"+value+"\n", true);
                                                        sent = true;
                                                        count = 0;
                                                        Files.creaEscribeDataArchivo4690(tslSeekAuxFileName, value, true);
                                                    }
                                                }
						...
						posFileSeekWriter.write(valorPosicion.getBytes(), 0, POSFile.FROM_START_OF_FILE, POSFile.FLUSH,
								valorPosicion.length());
                                        ...
                                        else  if((null == linea || linea.trim().equals("")) && punteroFile == randFileRead.length()&& !getEamtranFileName().equals(obtieneNombreSeek()) && storeClosedPassed){
                                            log.info("Linea null o vacia punteros iguales y diferentes EamTram" );
                                            log.info(getEamtranFileName() + " -- " +obtieneNombreSeek() );
                                            break;
                                        }
                                        else{
						logguerCount--;
				...
				if (isStoreClosed) {
					Files.creaEscribeDataArchivo4690(getEyesFileName(), "TSL_READ_P|"+properties.getHostName()+"|1|"+properties.getHostAddress()+"|"+storeNumber+"|PRC|"+ArmsAgentConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|Cierre tienda encontrado.\n", true);
	...
	protected boolean connectClient() {
		...
		log.info(socketClient.getIpServer());
	...
	protected String getEamtranFileName() {
		...
		try {
			...
			if (null != dataFile) {
				...
				eamtranFileName = properties.getObject("fileUpdaterUp.out.folder.copy.path") + File4690.separatorChar + eamtranFilePrefix + eamtranLetter + ".DAT";
				log.info("Nombre Tlog actual: " + eamtranFileName);
				businessDateDay = readFechaContable();
			} else
	...
	protected Frame leerRespuesta(ConnSocketClientProxy socketClient) {
