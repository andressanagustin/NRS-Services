...
public class RegistroPersonasEnColaOperation extends AbstractOperation {
	...
	protected String personasCola, numeroTrx, fecha, storeNumber, terminalNumber ;
	...
	protected void initialize() {
		log = Logger.getLogger(RegistroPersonasEnColaOperation.class);
		log.info("Inicia operacion Registro Personas en Cola V0");
		iniciarSesion("Arts");
	...
	public boolean process(ConnSocketServer socket, Frame frame, PropFile properties) {
		...
		log.debug("terminalNumber : " + terminalNumber);
		numeroTrx = (String) frame.getBody().get(2);
		log.debug("numeroTrx      : " + numeroTrx);
		personasCola = (String) frame.getBody().get(3);
		log.debug("personasCola   : " + personasCola);
		...
		log.debug("is null?" + (workstation == null));
		terminalCode = workstation.getWorkstationID();
		log.debug("terminalCode   : " + terminalCode);
		boolean grabo = false;
		try {
			grabo = insertaQueue(String.valueOf(storeCode), String.valueOf(terminalCode),numeroTrx, fecha, personasCola);
		...
		if (grabo) {
			msg.append(frame.getHeaderStr()).append(frame.getSeparator()).append("0"); // si salió bien
			log.debug("grabo en bdd");
					alertaSupervisor.lanzarAlertaColaExcedida(storeCode, terminalCode, Integer.valueOf(numeroTrx), Integer.valueOf(personasCola), null);


		} else {
			msg.append(frame.getHeaderStr()).append(frame.getSeparator()).append("1"); // no salió bien
			log.debug("No grabo en bdd");
		}

		// fin respuesta al CC
//		} catch (Exception e) {
//			log.info("Error al procesar: " + e);
//			msg.append(frame.getHeaderStr()).append(frame.getSeparator()).append("1"); // no salió bien
//		}
	...
	public boolean insertaQueue(String codigoTienda, String codigoTerminal, String numeroTrx,String fecha, String numeroCola)
			throws SQLException {
