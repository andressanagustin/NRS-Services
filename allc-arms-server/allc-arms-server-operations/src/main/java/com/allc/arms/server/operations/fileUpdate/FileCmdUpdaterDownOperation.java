...
public class FileCmdUpdaterDownOperation extends AbstractOperation{
	
	private Logger logger = Logger.getLogger(FileCmdUpdaterDownOperation.class);
	...
	public boolean process(ConnSocketServer socket, Frame frame, PropFile properties) {
		...
		logger.info("Iniciando FileCmdUpdaterDownOperation...");
		...
		try {
			...
			inFolder = new File(properties.getObject("SUITE_ROOT")+File.separator+paravalue.getValor()+File.separator+storeCode+File.separator+properties.getObject("fileUpdaterDown.folder.name")+File.separatorChar+"CmdFiles");
			
			UtilityFile.createWriteDataFile(getEyesFileName(properties),
					"UPD_FILEC_O|" + properties.getHostName() + "|3|" + properties.getHostAddress() + "|"
							+ frame.getHeader().get(3) + "|STR|"
							+ ArmsServerConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())
							+ "|Iniciando borrado de los archivos: " + frame.getBody().get(0)+ ".\n",
					true);
			...
			if(socket.writeDataSocket(tmp)){
				UtilityFile.createWriteDataFile(getEyesFileName(properties), "UPD_FILEC_O|"+properties.getHostName()+"|3|"+properties.getHostAddress()+"|"+frame.getHeader().get(3)+"|END|"+ArmsServerConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|Archivos: "+frame.getBody().get(0)+" eliminado.\n", true);
			} else
				UtilityFile.createWriteDataFile(getEyesFileName(properties), "UPD_FILEC_O|"+properties.getHostName()+"|3|"+properties.getHostAddress()+"|"+frame.getHeader().get(3)+"|WAR|"+ArmsServerConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|No se pudo enviar la respuesta.\n", true);
			...
			try {
				UtilityFile.createWriteDataFile(getEyesFileName(properties),
						"UPD_FILEC_O|" + properties.getHostName() + "|3|" + properties.getHostAddress() + "|"
								...
								+ ArmsServerConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())
								+ "|Error al intentar borrar el archivo " + frame.getBody().get(0) +".\n",
						true);
			} catch (Exception e1) {
