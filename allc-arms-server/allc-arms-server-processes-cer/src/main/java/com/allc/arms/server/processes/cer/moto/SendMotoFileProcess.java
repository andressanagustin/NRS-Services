 ...
 *
 */
public class SendMotoFileProcess extends AbstractProcess {
	private static Logger log = Logger.getLogger(SendMotoFileProcess.class);
	...
	protected void inicializar() {
		isEnd = false;
		try {
			inFolder = new File(properties.getObject("sendMoto.in.folder.path"));
			outFolder = new File(properties.getObject("sendMoto.out.folder.path"));
			serverFolder = new File(properties.getObject("sendMoto.server.folder.path"));
			sleepTime = properties.getInt("sendMoto.sleeptime");
			username = properties.getObject("sendMoto.username");
			password = properties.getObject("sendMoto.password");
			port = properties.getInt("sendMoto.server.port");
		} catch (Exception e) {
	...
	public void run() {
		log.info("Iniciando SendMotoFileProcess...");
		inicializar();
		...
		while (!isEnd) {
			...
			try {
				...
				if (motoFile != null) {
					iniciaOperacion();
					String filename = motoFile.getName().toUpperCase();
					...
					if (done) {
						...
						motoFile.renameTo(new File(outFolder, motoFile.getName()));
				...
				if (files.length == 0) {
					...
					Arrays.sort(files, new Comparator() {
						public int compare(Object obj1, Object obj2) {
							...
							int sequence2 = 0;
							if (name1.startsWith("LOTE") && !name2.startsWith("LOTE"))
								return -1;
							if (!name1.startsWith("LOTE") && name2.startsWith("LOTE"))
								return 1;
	...
	public boolean shutdown(long timeToWait) {
		...
		log.info("Deteniendo SendMotoFileProcess...");
		while (!finished)
			try {
		...
		log.info("Finalizó el Proceso de Envío de Archivos de Moto.");
		return true;
	}
