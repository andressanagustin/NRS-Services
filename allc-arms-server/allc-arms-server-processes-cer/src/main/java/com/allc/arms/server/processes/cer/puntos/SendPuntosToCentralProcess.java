package com.allc.arms.server.processes.cer.puntos;
...
public class SendPuntosToCentralProcess extends AbstractProcess {
	
	protected Logger log = Logger.getLogger(SendPuntosToCentralProcess.class);
	...
	protected boolean init() {
		...
		try {
			...
			Files.creaEscribeDataArchivo(getEyesFileName(),
					"SND_PTS_P|" + properties.getHostName() + "|1|" + properties.getHostAddress() + "|" + storeNumber
							+ "|STR|" + ArmsServerConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())
							+ "|Iniciando proceso de lectura del archivo de puntos en ArmsServer Local.\n",
					true);
			...
			puntosSeekFileName = "puntosFromServerSeek.dat";
			timeSleep = 15000;
	...
	public void run() {
		try {
			if (init()) {
				...
				Files.creaEscribeDataArchivo(getEyesFileName(),
						"SND_PTS_P|" + properties.getHostName() + "|1|" + properties.getHostAddress() + "|"
								+ storeNumber + "|END|"
								+ ArmsServerConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())
								+ "|Finaliza el proceso de envío del archivo de tramas de puntos.\n",
						true);
			} else
				Files.creaEscribeDataArchivo(getEyesFileName(),
						"SND_PTS_P|" + properties.getHostName() + "|1|" + properties.getHostAddress() + "|"
								+ storeNumber + "|ERR|"
								+ ArmsServerConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())
								+ "|Error al iniciar el proceso de envío del archivo de tramas de puntos.\n",
						true);
		} catch (Exception e) {
			try {
				Files.creaEscribeDataArchivo(getEyesFileName(),
						"SND_PTS_P|" + properties.getHostName() + "|1|" + properties.getHostAddress() + "|"
								+ storeNumber + "|ERR|"
								+ ArmsServerConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())
								+ "|Error en el proceso de envío de tramas de puntos.\n",
						true);
			} catch (Exception e1) {
	...
	public boolean shutdown(long timeToWait) {
		...
		log.info("Deteniendo SendPuntosToCentralProcess...");
		while (!finished)
			try {
	...
	protected boolean readTramas() {
		...
		try {
			puntosFileName = "PtosXArt"+ actualDateDay + ".dat";
			if (puntosFileName == null || puntosFileName.equals(""))
				return false;
			if (!Files.fileExists(puntosSeekFileName)) {
				Files.creaEscribeDataArchivo(puntosSeekFileName, valorEnCero, false);
				Files.creaEscribeDataArchivo(puntosSeekFileName,
						puntosFileName + ArmsServerConstants.Communication.CRLF, true);
			} else {
				puntosFileName = Files.readSpecifictLineOfFile(puntosSeekFileName, 2);
				if(puntosFileName == null){
					Files.deleteFile(puntosSeekFileName);
					return false;
				}
				actualDateDay = puntosFileName.substring(8, 16);
			}

			randSeekRead = new RandomAccessFile(puntosSeekFileName, "r");
			punteroFile = obtieneOffsetSeek(puntosSeekFileName);
			log.info("puntero:" + punteroFile);
			File file = new File(puntosFileName);
			if (!Files.fileExists(puntosFileName))
				Files.creaEscribeDataArchivo(puntosFileName, "", true);
			if (punteroFile >= 0) {
				randFileRead = new RandomAccessFile(puntosFileName, "r");
					...
					if (null != linea && !linea.trim().equals("")) {
						...
						if (sent) {
							...
							valorPosicion = Util.rpad(String.valueOf(punteroFile),
									...
									+ ArmsServerConstants.Communication.CRLF;
							Files.creaEscribeDataArchivo(puntosSeekFileName, valorPosicion, false);
							Files.creaEscribeDataArchivo(puntosSeekFileName,
									puntosFileName + ArmsServerConstants.Communication.CRLF, true);
						}
					} else {
		...
		try {
			...
			if (socketClient != null && socketClient.isConnected() && socketClient.writeDataSocket(trama)){
				UtilityFile.createWriteDataFile(getEyesFileName(),
						"SND_PTS_P" + properties.getHostName() + "|3|" + properties.getHostAddress() + "|"
