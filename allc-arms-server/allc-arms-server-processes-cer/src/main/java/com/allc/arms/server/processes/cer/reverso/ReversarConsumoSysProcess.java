 ...
 *
 */
public class ReversarConsumoSysProcess extends AbstractProcess {
	private static Logger log = Logger.getLogger(ReversarConsumoSysProcess.class);
	...
	protected void inicializar() {
		isEnd = false;
		try {
			inFolder = new File("C:/ALLC/WWW/EYES/RVSCER/IN");
			inFolder.mkdirs();
			outFolder = new File("C:/ALLC/WWW/EYES/RVSCER/BKP");
			outFolder.mkdirs();
	...
	public void run() {
		log.info("Iniciando ReversarConsumoProcess...");
		inicializar();
		...
		while (!isEnd) {
			try {
				...
				if (giftFile != null) {
					...
					UtilityFile.createWriteDataFile(getEyesFileName(), "RVS_CON_P|"+properties.getHostName()+"|3|"+properties.getHostAddress()+"|"+storeCode+"|STR|"+ArmsServerConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|Archivo a Procesar: "+filename+".\n", true);
					if (reversarConsumo(giftFile)) {
						giftFile.renameTo(new File(outFolder, giftFile.getName()));
						log.info("Archivo movido a la carpeta Out.");
						UtilityFile.createWriteDataFile(getEyesFileName(), "RVS_CON_P|"+properties.getHostName()+"|3|"+properties.getHostAddress()+"|"+storeCode+"|END|"+ArmsServerConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|Archivo: "+filename+" procesado correctamente.\n", true);
					} else {
						UtilityFile.createWriteDataFile(getEyesFileName(), "RVS_CON_P|"+properties.getHostName()+"|3|"+properties.getHostAddress()+"|"+storeCode+"|ERR|"+ArmsServerConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|Error al procesar el Archivo: "+filename+".\n", true);
						try {
	...
	private boolean reversarConsumo(File pagoFile) {
...
//		bitmap.setCharAt(RESERVED - 1, '1');
		try {
			...
			if (linea != null && linea.length() == 97) {
				...
				String expirationDate = generarNfijo("9912", 4);//lineParts[2].substring(18, 22), 4);//
...
//				String track1 = generarAnsvariable(lineParts[0], 76, 2);
				...
				//122
				String cupones = br.readLine();
				String addData2 = "";
				if(cupones != null && cupones.length() > 0){
					bitmap.setCharAt(ADICIONAL_DATA_5 - 1, '1');
					String cant = Integer.valueOf(cupones.length()/6).toString();
					if(cant.length() == 1)
						cant = "0" + cant;
					addData2 = generarAnsvariable(cant + cupones, (cant + cupones).length(), 4);
					msg.append(ArmsServerConstants.Communication.FRAME_SEP).append("AD5="+cant + cupones);
				}
				String totalPtosStr = "000000000";
				//125
				String puntos = br.readLine();
				log.info("Puntos: "+puntos);
				String reserved = "";
				if(puntos != null && puntos.length() > 0){
					bitmap.setCharAt(RESERVED - 1, '1');
					int cant = puntos.length()/11;
					String cantStr = Integer.valueOf(cant).toString();
					int totalPtos = 0;
					for(int i = 0; i < cant; i++){
						int ptosLine = i * 11;
						totalPtos += Integer.valueOf(puntos.substring(ptosLine + 2, ptosLine + 11));
					}
					totalPtosStr = totalPtosStr + Integer.valueOf(totalPtos).toString();
					if(cantStr.length() == 1)
						cantStr = "0" + cantStr;
					reserved = generarAnsvariable(cantStr + puntos, (cantStr + puntos).length(), 4);
					msg.append(ArmsServerConstants.Communication.FRAME_SEP).append("RV="+cantStr + puntos);
				}
				
				//124
				String resPrivUse = generarAnsvariable("P"+linea.substring(63, 88)+"          "+linea.substring(88,  97)+ totalPtosStr.substring(totalPtosStr.length()-9), 54, 4);//P0000000027081001020161143          000000146000000001", 54, 4);
				msg.append(ArmsServerConstants.Communication.FRAME_SEP).append("RFPU="+"P"+linea.substring(63, 88)+"          "+linea.substring(88,  97)+ totalPtosStr.substring(totalPtosStr.length()-9));
				mje.append(header);
				mje.append(type);
...
//				mje.append(track1);
	...
	//			devolucionDAO.insertGiftcard(session, giftcard);
				...
				log.info("Archivo con formato incorrecto");
			br.close();
			fr.close();
		} catch (Exception e) {
			log.error(e.getMessage(), e);
	...
	private String createMsg(String data) {
		...
		if ((bitmap.charAt(BITMAP_2_ACTIVE - 1) == '1') && bitmap.charAt(ADICIONAL_DATA_5 - 1) == '1') {
			int cantAleer = Integer.valueOf(data.substring(pos, pos + 4)).intValue();
			String addData = data.substring(pos+4, pos+4 +((cantAleer * 2)));
			msg.append(ArmsServerConstants.Communication.FRAME_SEP).append("AD5="+translateHexaStringToUtf8(addData));
			pos = pos + 4 + (cantAleer * 2);
		}
...
//		log.info("Respuesta: " + retorno.toString());
	...
	private boolean respuestaAfirmativa(String data) {
		StringBuffer bitmap = translateHexaToBitmap(data.substring(14, 30));
		log.info("BitMap1: "+data.substring(14, 30) +" RA");
		int pos = 30;
		if (bitmap.charAt(BITMAP_2_ACTIVE - 1) == '1') {
			pos += 16;
			log.info("BitMap2: "+data.substring(30, pos) +" RA");
			...
			do {
				...
				File[] files = inFolder.listFiles(new FileFilter() {
					public boolean accept(File pathname) {
						return pathname.isFile() && (pathname.getName().toUpperCase().startsWith("CON"));
					}
				});
				if (files.length == 0) {
					try {
