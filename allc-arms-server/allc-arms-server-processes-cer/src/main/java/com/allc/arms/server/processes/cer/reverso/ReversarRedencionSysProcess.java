 ...
 *
 */
public class ReversarRedencionSysProcess extends AbstractProcess {
	private static Logger log = Logger.getLogger(ReversarRedencionSysProcess.class);
	...
	protected void inicializar() {
		isEnd = false;
		try {
			inFolder = new File("C:/ALLC/WWW/EYES/RVSRED/IN");
			inFolder.mkdirs();
			outFolder = new File("C:/ALLC/WWW/EYES/RVSRED/BKP");
			outFolder.mkdirs();
	...
	public void run() {
		log.info("Iniciando ReversarRedencionSysProcess...");
		inicializar();
		...
		while (!isEnd) {
			try {
				...
				if (giftFile != null) {
					...
					UtilityFile.createWriteDataFile(getEyesFileName(), "RVS_RED_P|"+properties.getHostName()+"|3|"+properties.getHostAddress()+"|"+storeCode+"|STR|"+ArmsServerConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|Archivo a Procesar: "+filename+".\n", true);
					if (reversarConsumo(giftFile)) {
						giftFile.renameTo(new File(outFolder, giftFile.getName()));
						log.info("Archivo movido a la carpeta Out.");
						UtilityFile.createWriteDataFile(getEyesFileName(), "RVS_RED_P|"+properties.getHostName()+"|3|"+properties.getHostAddress()+"|"+storeCode+"|END|"+ArmsServerConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|Archivo: "+filename+" procesado correctamente.\n", true);
					} else {
						UtilityFile.createWriteDataFile(getEyesFileName(), "RVS_RED_P|"+properties.getHostName()+"|3|"+properties.getHostAddress()+"|"+storeCode+"|ERR|"+ArmsServerConstants.DateFormatters.yyyyMMddHHmmss_format.format(new Date())+"|Error al procesar el Archivo: "+filename+".\n", true);
						try {
	...
	private boolean reversarConsumo(File pagoFile) {
		...
		try {
			...
			if (linea != null && linea.length() == 97) {
				...
				//124
				String resPrivUse = generarAnsvariable("P"+linea.substring(63, 88)+"          000000000"+StringUtils.leftPad(linea.substring(88), 9, ArmsServerConstants.Communication.CERO), 54, 4);
				msg.append(ArmsServerConstants.Communication.FRAME_SEP).append("RFPU="+"P"+linea.substring(63, 88)+"          000000000"+StringUtils.leftPad(linea.substring(88), 9, ArmsServerConstants.Communication.CERO));
				mje.append(header);
				mje.append(type);
			...
			do {
				...
				File[] files = inFolder.listFiles(new FileFilter() {
					public boolean accept(File pathname) {
						return pathname.isFile() && (pathname.getName().toUpperCase().startsWith("RED"));
					}
				});
				if (files.length == 0) {
					try {
