...
public class AlertasTiempoEsperaColaTiendasProcess extends AbstractProcess {
	...
	public void run() {
		while (!isEnd) {
			try {
				verificarTecs();
				Thread.sleep(properties.getLong("colas.alertas.tec.timesleep"));
	...
	public void verificarTecs() {
		Integer minutosIntervalo =  properties.getInt("colas.alertas.tec.timesleep")/1000/60;
		String spBd = "select * from colas_ec.fn_alerta_tec_tienda(:intervalo)";
