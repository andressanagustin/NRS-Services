...
{
	static Logger log = Logger.getLogger(ThreadUtilities.class);
// Dummy constructor
	/**
...
// Thread groups
	...
	public static ThreadGroup getThreadGroup( final String name )
	{
		...
		for(int i=0; i < list.size();i++	){
			if(((ThreadGroup)list.get(i)).getName().equals(name))
				return (ThreadGroup)list.get(i);
		}
		return null;
	}
...
/*	public static <T,U> T[] copyOf(U[] original, int newLength, Class<? extends T[]> newType) {
	...
	private static Thread[] copyOf(Thread[] original, int newLength){
		Thread[] ThreadTmp = new Thread[newLength];
		
		if( original.length < newLength)
			Arrays.fill(ThreadTmp, 0, newLength -1, null);
		
		System.arraycopy(original, 0, ThreadTmp, 0, Math.min(original.length, newLength));
...
/*	public static Thread getBlockingThread( final Thread blockedThread )
	...
	public static void threadReborn(String groupName){
		try {
			...
			//while(true){
				...
				//Thread.sleep(L)
				Thread[] tarray = ThreadUtilities.getGroupThreads(groupName);
				...
				for(int i=0; i < list.size();i++){
					...
					//if(!((Thread)list.get(i)).isAlive()){
						/**Lanzamos el thread para verificar que esten vivos los otros procesos****/
