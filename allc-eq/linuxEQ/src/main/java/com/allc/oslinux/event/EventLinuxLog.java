...
public class EventLinuxLog {

	static Logger log = Logger.getLogger(EventLinuxLog.class);
	static RandomAccessFile randStore = null;
	EventLinux eventWin;
	/**Constructor**/
	public EventLinuxLog(EventLinux eventWin) {
...
/*	*//**
	...
	*//**
	 ...
	 */
	public boolean storedWinLog(Saf saf, EventLinux event, EventLogRecord record){
		...
		try {
			 ...
			 data = /**Communication Channel**/
				   ...
				   /**Terminal ID**/
				   PropertiesLinuxEQ.Parameters.TERMINAL_ID + saf.getCar() + 
				   /**Message Group**/
	 ...
	 */
	public void registerEvents(EventLinux eventWin){
		List<String> list;
		...
		try {
			...
			if(Files.fileExists(PropertiesLinuxEQ.ParamLinuxEQ.FILE_SEEK_LINUX_LOG)){
				...
				hash = PropertyFileLoader.getProperties(PropertiesLinuxEQ.ParamLinuxEQ.FILE_SEEK_LINUX_LOG);
			...
			list = Arrays.asList(P.split(PropertiesLinuxEQ.Parameters.EVENT_SOURCE));
			/**set the data level to obtain**/
			//EventLogIterator iter = new EventLogIterator("Application");
			//iter = new EventLogIterator("SRVSTSAND01", PropertiesWinEQ.Parameters.EVENT_TYPES , route);
			iter = new EventLogIterator(PropertiesLinuxEQ.Parameters.COMPUTER_SOURCE==""?null:PropertiesLinuxEQ.Parameters.COMPUTER_SOURCE, PropertiesLinuxEQ.Parameters.EVENT_GROUP , route);
			/**Iteration**/
			while(iter.hasNext()) {
			    ...
			    if(isRecordRegistered){
			    	...
			    	saveHashMapIntoFile(hash, PropertiesLinuxEQ.ParamLinuxEQ.FILE_SEEK_LINUX_LOG);
			...
			if( getLastRecordDate(hash).getTime() > lastRecordDate.getTime() ){
				...
				saveHashMapIntoFile(hash, PropertiesLinuxEQ.ParamLinuxEQ.FILE_SEEK_LINUX_LOG);
				log.info("Hashmap save into file: " + PropertiesLinuxEQ.ParamLinuxEQ.FILE_SEEK_LINUX_LOG);
	 ...
	 * @param record
	 */
	private boolean registerWinLogEvent(EventLinux eventWin, List<String> list,	Saf saf, EventLogRecord record) {
		boolean registro = false;
		/**if we want all events (*) **/
		if(PropertiesLinuxEQ.Parameters.EVENT_SOURCE.equals("*")){
	...
	public static boolean loadFileIntoHashMap(String FileName, HashMap<String, String> hashmap){
		boolean exito = false;
		try {
			hashmap = PropertyFileLoader.getProperties(PropertiesLinuxEQ.ParamLinuxEQ.FILE_SEEK_LINUX_LOG);
			exito = true;
		} catch (Exception e) {
